...
static void iot_timer_create(hw_timer_idx_t *timer_id, bool auto_reload, uint32_t timer_interval_ms, void *isr_handle) {
    ...
    timer_isr_register(timer_id->timer_group, timer_id->timer_id, isr_handle, (void *) timer_id->timer_id, ESP_INTR_FLAG_IRAM, NULL);
}
...
static IRAM_ATTR esp_err_t iot_ledc_duty_config(ledc_mode_t speed_mode, ledc_channel_t channel, int hpoint_val, int duty_val, uint32_t duty_direction, uint32_t duty_num, uint32_t duty_cycle, uint32_t duty_scale) {
    if (hpoint_val >= 0) {
    ...
    LEDC.channel_group[speed_mode].channel[channel].conf1.val = ((duty_direction & LEDC_DUTY_INC_LSCH0_V) << LEDC_DUTY_INC_LSCH0_S) | ((duty_num & LEDC_DUTY_NUM_LSCH0_V) << LEDC_DUTY_NUM_LSCH0_S) | ((duty_cycle & LEDC_DUTY_CYCLE_LSCH0_V) << LEDC_DUTY_CYCLE_LSCH0_S) | ((duty_scale & LEDC_DUTY_SCALE_LSCH0_V) << LEDC_DUTY_SCALE_LSCH0_S);
    ...
    for (int channel = 0; channel < LEDC_CHANNEL_MAX; channel++) {
        ...
        if (fade_data->num > 0) {
            ...
            if (fade_data->step) {
                ...
                if (fade_data->num != 0) {
                    _iot_set_fade_with_time(g_light_config->speed_mode, channel, gamma_value_to_duty(fade_data->cur), DUTY_SET_CYCLE - LEDC_FADE_MARGIN);
                } else {
...
esp_err_t iot_led_regist_channel(ledc_channel_t channel, gpio_num_t gpio_num) {
    ...
    LIGHT_ERROR_CHECK(gpio_num != GPIO_NUM_16 || gpio_num != GPIO_NUM_17, ESP_ERR_INVALID_ARG, "gpio_num must not conflict to PSRAM(IO16 && IO17)");
#endif
