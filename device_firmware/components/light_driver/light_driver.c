...
esp_err_t light_driver_breath_start(uint8_t red, uint8_t green, uint8_t blue) {
    ...
    ret = iot_led_start_blink(CHANNEL_ID_RED, red, g_light_status.blink_period_ms, true);
    LIGHT_ERROR_CHECK(ret < 0, ESP_FAIL, "iot_led_start_blink, ret: %d", ret);
    ret = iot_led_start_blink(CHANNEL_ID_GREEN, green, g_light_status.blink_period_ms, true);
    LIGHT_ERROR_CHECK(ret < 0, ESP_FAIL, "iot_led_start_blink, ret: %d", ret);
    ret = iot_led_start_blink(CHANNEL_ID_BLUE, blue, g_light_status.blink_period_ms, true);
    LIGHT_ERROR_CHECK(ret < 0, ESP_FAIL, "iot_led_start_blink, ret: %d", ret);
...
esp_err_t light_driver_fade_brightness(uint8_t brightness) {
    ...
    if (g_light_status.mode == MODE_HSV) {
        ...
        ret = iot_led_set_channel(CHANNEL_ID_COLD, cold_tmp * 255 / 100, fade_period_ms);
        LIGHT_ERROR_CHECK(ret < 0, ret, "iot_led_set_channel, ret: %d", ret);
        ret = iot_led_set_channel(CHANNEL_ID_WARM, warm_tmp * 255 / 100, fade_period_ms);
        LIGHT_ERROR_CHECK(ret < 0, ret, "iot_led_set_channel, ret: %d", ret);
