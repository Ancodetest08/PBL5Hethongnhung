...
static esp_err_t read_random_bytes_from_nvs(uint8_t **random_bytes, size_t *len) {
    ...
    *len = 0;
    ...
    if ((err = nvs_open_from_partition(CONFIG_ESP_RMAKER_FACTORY_PARTITION_NAME, CREDENTIALS_NAMESPACE, NVS_READONLY, &handle)) != ESP_OK) {
...
static esp_err_t get_device_service_name(char *service_name, size_t max) {
    ...
    if ((read_random_bytes_from_nvs(&nvs_random, &nvs_random_size) != ESP_OK) || nvs_random_size < 3) {
        ...
        snprintf(service_name, max, "%s%02x%02x%02x", ssid_prefix, nvs_random[nvs_random_size - 3], nvs_random[nvs_random_size - 2], nvs_random[nvs_random_size - 1]);
    }
